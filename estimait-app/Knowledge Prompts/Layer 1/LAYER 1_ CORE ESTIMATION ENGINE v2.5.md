...
- **[NEW v2.6] Public Works Router**: Full routing to LAYER2_PUBLIC_WORKS on detection.
- **[NEW v2.6] Sanity Check**: Added Steps 9 & 10 for public works validation.
- **[NEW v2.6] on_stage_1_complete**: Enhanced public works detection + override logic.
...
*   **`on_stage_1_complete`**: Ingest validated Tier 1 cost drivers. **Classify project as "New Build" or "Renovation"** AND **"Public" vs. "Private"**. If Renovation, determine renovation scope (Light/Moderate/Heavy). **Calculate Project Complexity Score**.
    ...
    **[NEW v2.6] PUBLIC WORKS ROUTING LOGIC:**
    ```
    IF PUBLIC_WORKS == TRUE (detected by STAGE A keyword scan):

      STEP 1 — Load knowledge:
        → get_knowledge_prompt("LAYER2", building_type="PUBLIC_WORKS")
        → This overrides default LAYER2 building type for cost parameters

      STEP 2 — Override private defaults:
        Labor_Rates        → Prevailing Wage (state/federal, see PW Section 5)
        Bond_Insurance     → 2.5%–3.5% (vs 1.0%–1.5% private)
        Mobilization       → Separate Pay Item 4%–10% (vs embedded in Div 01)
        Contingency        → 8%–15% (vs 5%–10% private)
        Admin_Overhead     → 2.5%–5.0% (vs 0.5%–1.0% private)
        OH_P               → Nuisance Premium table (PW Section 4)

      STEP 3 — Trigger mandatory checks:
        → DBE_check:              IF DBE_Goal > 0% → calculate DBE premium
        → Prevailing_Wage:        Confirm state + trade classifications
        → Federal_Aid_check:      IF BRLO/STIP/FTA/FHWA → load federal overlay
        → Multi_site_mob:         IF N_sites > 1 → apply multi-site formula
        → Seasonal_constraints:   Scan for irrigation/fish/winter/school windows
        → Liquidated_Damages:     Confirm LD rate → calculate schedule risk premium
        → Buy_America:            IF federal-aid → add material premium 5%–12%

      STEP 4 — Activate public works intake:
        → Present 10-question PUBLIC WORKS INTAKE CHECKLIST (PW Section 7)
        → DO NOT proceed to Stage 2 until all 10 answers received
...
### **Step 9: Public Works Multiplier Validation (PUBLIC WORKS ONLY)**

*Trigger: Execute only when `PUBLIC_WORKS == TRUE`*

Verify total bid includes ALL mandatory public works cost items:
```
  ✅ Prevailing wage labor rates (NOT open-shop)
  ✅ Performance & Payment Bonds (0.5%–2.0% of contract)
  ✅ Enhanced insurance (total bond + insurance = 2.5%–3.5%)
  ✅ Mobilization as separate Pay Item (4%–10%, or multi-site formula)
  ✅ DBE compliance premium (IF DBE_Goal > 0%)
  ✅ Federal-aid admin overhead (IF federal-funded: +2.5%–5.0%)
  ✅ Liquidated damages risk premium (IF LD > $2,000/day)
  ✅ Seasonal restart costs (IF seasonal constraints detected)
  ✅ Buy America material premium (IF FHWA/FTA/federal: +5%–12%)
  ✅ Traffic control (IF road/bridge work)

Benchmark ratio check:
  Public_Total ÷ Equivalent_Private_Total = expected 1.25x–1.50x
  IF ratio < 1.20: ⚠️ FAIL — LIKELY MISSING PUBLIC WORKS COST ITEMS
  IF ratio > 1.60: ⚠️ CAUTION — VERIFY NO DOUBLE-COUNTING
```

### **Step 10: Unit Price Completeness Check (DBB UNIT PRICE CONTRACTS ONLY)**

*Trigger: Execute only when `Bid_Format == UNIT_PRICE`*

```
For each unit price line item on bid schedule:
  □ Unit is correct (CY / SY / LF / EA / LS / SF)?
  □ Quantity is taken from official bid schedule (not self-estimated)?
  □ Prevailing wage labor is embedded in unit rate?
  □ Material source risk is considered (delivery lead times, escalation)?
  □ Production rate is achievable given site conditions and season?
  □ Unbalanced bid risk reviewed (if front-loading: max 15%–20%)?

IF any box unchecked → flag item as ⚠️ REQUIRES CONFIRMATION before submitting
```
